...
const cvsFiltration = (arr) => {
    ...
    let newArr=[];
    arr.filter(person => {
        let fullName=  person.firstName + (person.LastName ? " " + person.LastName : "");
        if(person.tech== "JS" && person.yearsOfExperience > 4)
        {
            newArr.push({fullName:fullName,tech:person.tech});
        }
    })
    return newArr;
}
