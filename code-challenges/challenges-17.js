...
const filterLinks = (str) => {
    ...
    let end = str.indexOf('.com', start);

  if (end === -1) {
    end = str.indexOf('.org', start);
  }

  if (end === -1) {
    end = str.indexOf('.net', start);
  }

  if (start !== -1 && end !== -1) {
    let link = str.slice(start, end + 4);
    return link;
  }
...
const isPalindrome = (str) => {
    // write your code here
    let sentence=str.toLowerCase().replace(/[^a-z0-9]/g, '');
    let palindrome=sentence.split("").reverse().join("");
